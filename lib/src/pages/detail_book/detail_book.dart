...
class _DetailBookPageState extends State<DetailBookPage> {
  ...
  void initState() {
    ...
    _itemBookBloc.add(
        GetItemBookEvent(widget.bookModel.eTitBib!.eBib!.bibId.toString()));
    super.initState();
  }
            ...
            child: ListView(
              ...
              children: [
                ...
                BookInformation(
                    ...
                    information: parsingBook.parsingAuthor(widget
                        .bookModel.eTitBib!.eBib!.eAutBib!.eAut!.autKey!)),
                const SizedBox(height: 30),
                BlocBuilder<ItemBookBloc, ItemBookState>(
                  builder: (context, state) {
                    final callKey = widget.bookModel.eTitBib!.eBib!.calKey!;
                    if (state is ItemBookSuccessState) {
                      final rows = state.listItemBook
                          .map((e) => DataRow(cells: [
                                DataCell(Text(e.itemNo ?? "")),
                                DataCell(Text(parsingItemBook.parsingCopyNo(
                                    callKey, "${e.copyNo}"))),
                                DataCell(Text(parsingItemBook
                                ...
                                onSort: (columnIndex, sortAscending) {
                                  setState(() {
                                    if (columnIndex == _sortColumnIndex) {
                                      _sortAsc = _sortNoItemAsc = sortAscending;
                                    } else {
                                    ...
                                    if (!_sortAsc) {
                                      ...
                                      state.listItemBook =
                                          state.listItemBook.reversed.toList();
                                    }
                                  });
                                },
                              ),
                              DataColumn(
                                label: Text('No. Panggil'),
                                onSort: (columnIndex, sortAscending) {
                                  setState(() {
                                    if (columnIndex == _sortColumnIndex) {
                                      _sortAsc = _sortNoCallAsc = sortAscending;
                                    } else {
                                      _sortColumnIndex = columnIndex;
